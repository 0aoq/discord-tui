...
const loadMainApp = () => {
    ...
    screen.append(guilds)
    screen.append(channels)

    screen.append(titleBar)
    screen.append(topDropDownMenu)
 ...
 * @param {string} props.importantRoles An array of all roles that we need to look for in permissions
 */
const renderGuildChannels = async (props) => {
    ...
    const createItem = (channel) => {
        ...
        item.on('mouseover', () => { _saved = item.style.bg; item.style.bg = 'white'; screen.render() })
        item.on('mouseout', () => { item.style.bg = _saved; screen.render() })
...
const renderUserGuilds = async () => {
    ...
    for (let guild of guildList) {
        ...
        item.on('mouseover', () => { item.style.bg = 'white'; screen.render() })
        item.on('mouseout', () => { item.style.bg = ''; screen.render() })
        item.on('click', async () => {
...
titleBarOptions[1].on('click', () => {
    ...
    for (let item of socketLog) {
        ...
        _item.on('mouseover', () => { _item.style.bg = 'white'; screen.render() })
        _item.on('mouseout', () => { _item.style.bg = ''; screen.render() })
